...
1. To start the AMQP consumer on the host system when running the example on QEMU, run the following command:
   ```
...
1. To make sure you have the RabbitMQ message broker running, run the following command:
   ...
   ```
1. To start the AMQP consumer on the host system when running the example on Raspberry Pi 4, run the following command:
   ```
...
The AMQP publisher for KasperskyOS is built using the CMake build system, which is provided in the KasperskyOS Community Edition SDK.

In this example, an AMQP consumer must be started on the host operating system, and the AMQP publisher must be started on KasperskyOS.

Go to the directory with the AMQP publisher example and run the following command `./cross-build.sh <platform> [SDK_PATH]`, where:
...
```
The command builds the AMQP publisher example with the built-in RabbitMQ-C AMQP client library and runs the KasperskyOS-based solution image on QEMU. The solution image is based on the SDK found in the `/opt/KasperskyOS-Community-Edition-1.1.1.13` path.
...
[./publisher/CMakeLists.txt](publisher/CMakeLists.txt)—CMake commands for building the `Publisher` program.
